...
namespace structures
{
	...
	inline LinkedList<T>& LinkedList<T>::operator=(const LinkedList<T>& other)
	{
		if (this == &other)
			return *this;
		clear();
		LinkedListItem<T> * tmpItem = other.first_;
		while (tmpItem != nullptr) {
			add(tmpItem->accessData());
	...
	inline void LinkedList<T>::insert(const T & data, const int index)
	{
		...
		if (index == size_) {
			...
			if (index == 0) {
				...
				LinkedListItem<T> * curItem = getItemAtIndex(index - 1);
				newItem->setNext(curItem->getNext());
	...
	inline T LinkedList<T>::removeAt(const int index)
	{
		...
		if (index != 0) {
			current = getItemAtIndex(index - 1);
	...
	{
		LinkedListItem<T> * curItem = first_;
		while (curItem != nullptr) {
			first_ = curItem->getNext();
			delete curItem;
			curItem = first_;
		}
		size_ = 0;
	...
	{
		LinkedListItem<T> * tmpItem = first_;
